...
public class CategoryServiceImpl implements CategoryService {
    ...
    public void add(Category category) {
        ...
        saveCategoryTreeToRedis();//数据修改后，重新进行数据缓存预热
        logger.info("数据修改后，重新进行数据缓存预热");
    }
    ...
    public void delete(Integer id){
        ...
        if(count > 0){
            throw new RuntimeException("该分类存在下级分类，不能删除");
        }
    ...
    public List<Map> findCategoryTree() {
        //从缓存中提取商品分类树
        List<Map> categoryTree = (List<Map>)redisTemplate.boundValueOps(CacheKey.CATEGORY_TREE).get();
        logger.info("从缓存中提取商品分类树成功，categoryTree={}", JSON.toJSONString(categoryTree));
        return categoryTree;
    }
    ...
    public void saveCategoryTreeToRedis() {
        ...
        logger.info("将商品分类树放入缓存中，缓存预热会调用此方法");
    }
